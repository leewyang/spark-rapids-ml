...
class KMeans(KMeansClass, _CumlEstimator, _KMeansCumlParams):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.use_nccl = kwargs.pop("use_nccl") if "use_nccl" in kwargs else False
    ...
    def _get_cuml_fit_func(
        ...
        def _cuml_fit(
            ...
            if self.fake_pipe:
                ...
                if self.use_comms:
                    if self.use_nccl:
                        start_nccl = time.time()
                        print(f"===== start allReduce (NCCL): {ndev=}, {rank=}")
                        comm = nccl.NcclCommunicator(ndev, commId, rank)

                        data_min = cp.array(scaler.data_min_)
                        data_max = cp.array(scaler.data_max_)
                        data_size = len(data_min) * data_min.dtype.itemsize
                        # print(f"===== {len(data_min)=}, {data_min.dtype.itemsize=}, {data_size=}")
                        # print(f"===== {data_min=}")
                        # print(f"===== {data_max=}")

                        stream = cp.cuda.stream.get_current_stream()
                        comm.allReduce(data_min.data.ptr, data_min.data.ptr, data_size, nccl.NCCL_FLOAT32, nccl.NCCL_MIN, stream.ptr)
                        comm.allReduce(data_max.data.ptr, data_max.data.ptr, data_size, nccl.NCCL_FLOAT32, nccl.NCCL_MAX, stream.ptr)
                        stream.synchronize()

                        # print(f"===== {data_min=}")
                        # print(f"===== {data_max=}")
                        scaler = MinMaxScaler()
                        scaler.fit(cp.array([data_min, data_max]))
                        print("===== stop allReduce (NCCL): {}".format(time.time() - start_nccl))
                    else:
                        print("===== start allReduce (BarrierTaskContext")
                ...
                print("==== stop MinMaxScaler: {}".format((time.time() - start_scaler)))
