...
def test_spark_pipeline(gpu_number: int, tmp_path: str) -> None:
    ...
    with CleanSparkSession() as spark:
        ...
        kmeans = (
            KMeans().setK(2).setFeaturesCol("features").setPredictionCol("predictions")
        )
    ...
    with CleanSparkSession() as spark:
        ...
        kmeans = KMeans().setFeaturesCol("scaled").setPredictionCol("predictions")
...
def test_cuml_pipeline(gpu_number: int, tmp_path: str) -> None:
    ...
    #     df = spark.createDataFrame(data, ["features", "weighCol"])
    #     scaler = MinMaxScaler().setInputCol("features").setOutputCol("scaled")
    #     kmeans = KMeans().setK(2).setFeaturesCol("scaled").setPredictionCol("predictions")
